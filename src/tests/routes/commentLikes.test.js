...
describe('Comment likes', () => {
  ...
  it('Should not let the user create a comment like with and commentId that does not exist', (done) => {
    chai
      .request(app)
      .post(`/api/v1/articles/${createdArticle.slg}/comments/${createdComment.id}e/like`)
      .set('access-token', accessToken)
      .send()
      .end((err, res) => {
        res.should.have.status(status.NOT_FOUND);
